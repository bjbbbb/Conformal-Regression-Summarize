...
from torchcp.regression.predictors import CQR, SplitPredictor
...
def run_split_predictor(X_train, train_data_loader, cal_data_loader, test_data_loader, X_test, y_test, alpha, device,
                        epochs=100):
    print("运行Split方法...")
...
def visualize_predictions(predictions_file, run_number=None):
    ...
    plt.savefig(f"split_synthetic.pdf", dpi=300, bbox_inches='tight', format='pdf')
    print(f"split{run_suffix}.pdf")
    plt.close()
