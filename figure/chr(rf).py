...
def run_chr_rf(X_train, y_train, X_calib, y_calib, X_test, y_test, test_data_loader, alpha, device, n_estimators=100,
               min_samples_leaf=50):
    print("运行CHR(RF)方法...")
    grid_quantiles = np.arange(0.01, 1.0, 0.01)
    bbox = QRF(grid_quantiles, n_estimators=n_estimators, min_samples_leaf=min_samples_leaf, random_state=2020,
    ...
    save_test_predictions(model, test_data_loader, device)
...
def save_test_predictions(predictor, test_data_loader, device, run_number=None):
    ...
    for batch_x, batch_y in test_data_loader:
        ...
        batch_x_np = batch_x.cpu().numpy()  # 将Tensor转换为NumPy数组
        with torch.no_grad():
            batch_predictions = predictor.predict(batch_x_np)  # 使用NumPy数组进行预测
...
def visualize_predictions(predictions_file, run_number=None):
    ...
    plt.title(f'CHR(RF)', fontsize=32)
    ...
    plt.savefig(f"chr(rf)_synthetic.pdf", dpi=300, bbox_inches='tight', format='pdf')
...
def run_method_experiments(method_name, data_name, ratio_train, test_ratio, alpha, num_runs, device):
    ...
    for run in range(num_runs):
        ...
        try:
            ...
            if method_name == 'CHR(NN)':
                run_chr_rf(X_train, y_train, X_calib, y_calib, X_test, y_test, test_data_loader,
                           alpha, device)
