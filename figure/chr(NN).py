...
def run_chr_nn(X_train, y_train, X_calib, y_calib, X_test, y_test, test_data_loader, alpha, device, epochs=2000, lr=0.0005, dropout=0.1):
    print("运行CHR(NN)方法...")
    grid_quantiles = np.arange(0.01, 1.0, 0.01)
    # 注意：某些黑盒模型可能不支持直接GPU计算，在此我们采用CPU计算
...
def visualize_predictions(predictions_file, run_number=None):
    ...
    plt.title(f'CHR(NN)', fontsize=32)
    ...
    plt.savefig(f"chr(NN)_synthetic.pdf", dpi=300, bbox_inches='tight', format='pdf')
    print(f"chr(NN){run_suffix}.pdf")
    plt.close()
    ...
    for run in range(num_runs):
        ...
        try:
            ...
            if method_name == 'CHR(NN)':
                run_chr_nn(X_train, y_train, X_calib, y_calib, X_test, y_test, test_data_loader,
                           alpha, device)
