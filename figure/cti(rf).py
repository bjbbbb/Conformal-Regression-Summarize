...
def run_cti_rf(X_train, y_train, X_calib, y_calib, X_test, y_test, cal_data_loader, test_data_loader, alpha, device,
                n_estimators=100,
                min_samples_leaf=50):
    print("运行CHR(RF)方法...")
...
def visualize_predictions(predictions_file, run_number=None):
    ...
    for _, row in predictions_df.iterrows():
        ...
        for lower, upper in valid_intervals:
            ...
            # 找到最接近的网格索引
            lower_idx = max(0, min(grid_size-1, int((lower_bound - y_min) / y_step)))
            upper_idx = max(0, min(grid_size-1, int((upper_bound - y_min) / y_step)))
    ...
    plt.title(f'CTI(RF)', fontsize=32)
    ...
    plt.savefig(f"cti(rf)_synthetic.pdf", dpi=300, bbox_inches='tight', format='pdf')
    print(f"可视化结果已保存到 cti_rf.pdf")
    plt.close()
...
def run_method_experiments(method_name, data_name, ratio_train, test_ratio, alpha, num_runs, device):
    ...
    for run in range(num_runs):
        ...
        try:
            ...
            if method_name == 'CTI(RF)':
                run_cti_rf(X_train, y_train, X_calib, y_calib, X_test, y_test, cal_data_loader, test_data_loader,
                            alpha, device)
