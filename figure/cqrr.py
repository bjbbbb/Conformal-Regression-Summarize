...
from torchcp.regression.predictors import CQRR
...
def visualize_predictions(predictions_file, run_number=None):
    ...
    plt.savefig(f"cqrr_synthetic.pdf", dpi=300, bbox_inches='tight', format='pdf')
    print(f"cqrr{run_suffix}.pdf")
    plt.close()

def run_cqrr(X_train, train_data_loader, cal_data_loader, test_data_loader, X_test, y_test, alpha, device, epochs=100, run_number=None):
    print("运行CQRR方法...")
