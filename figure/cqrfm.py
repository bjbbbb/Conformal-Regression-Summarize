...
from torchcp.regression.predictors.cqrfm import CQRFM
...
def save_test_predictions(predictor, test_data_loader, device, run_number=None):
    ...
    file_name = f"cqrr_test_predictions{run_str}.csv"
...
def visualize_predictions(predictions_file, run_number=None):
    ...
    plt.savefig(f"cqrfm_synthetic.pdf", dpi=300, bbox_inches='tight', format='pdf')
    print(f"cqrfm{run_suffix}.pdf")
    plt.close()


def run_cqrfm(X_train, train_data_loader, cal_data_loader, test_data_loader, X_test, y_test, alpha, device, epochs=100):
    """
    训练并评估CQRFM方法。
    """
    print("运行CQRFM方法...")
    # 定义三个分位数：下分位数(alpha/2)、中位数(0.5)、上分位数(1-alpha/2)
    quantiles = [alpha / 2, 0.5, 1 - alpha / 2]

    # 构建神经网络模型
    model = build_regression_model("NonLinearNet")(X_train.shape[1], len(quantiles), 64, 0.5).to(device)
    # 使用分位数损失函数
